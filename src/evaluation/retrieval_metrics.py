...
# RECONCILIATION NOTE (mined onto main 2026-07-29)
# ------------------------------------------------
# `evaluation.recall_fp` is the CANONICAL implementation for anything that
# reaches the manuscript: it does per-query ranking over an embedding matrix,
# implements the adversarial strict tie-walk of Lin et al. 2023, locks its edge
# cases, and is covered by the recall_fp test suite and barrier spec.
#
# This module is the FLAT-VECTOR variant: given one already-computed ranked list
# of (distance, label) pairs it returns recall@1FP and AUROC. `classification_eval`
# needs that shape, which recall_fp does not currently expose.
#
# The two are therefore not interchangeable, and they do NOT share tie-handling
# semantics. Reconcile them (most likely by adding a flat-vector entry point to
# recall_fp and deleting this module) BEFORE any number from here is reported.
